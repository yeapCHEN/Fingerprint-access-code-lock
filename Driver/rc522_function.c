...
#define   macRC522_DELAY()  delay_us ( 3 )

int i=0;
char cStr [ 30 ];
extern u8 ucArray_ID [ 4 ];                                                                                             //先后存放IC卡的类型和UID(IC卡序列号)
u8 ucStatusReturn;                                                                                               //返回状态
u8 KEY_A[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; //4C4452464A48  4C4452464A48  4C444F50454E
u8 RC522rechive[16]={0}; 
...
{
     unsigned char res = 0;
		 ucStatusReturn=IC_CMT (ucArray_ID, KEY_A, 1, RC522rechive );
	   printf("%d\n\r",ucStatusReturn);
        ...
        for(i=0;i<16;i++)
       {
        if(i < 6)
				{						 
							ICrevbuf[2*i]	= RC522rechive[i]>>4;
							ICrevbuf[2*i + 1]	= RC522rechive[i]>>4;	
				}					
				printf("%x",RC522rechive[i]>>4);
				printf("%x",RC522rechive[i]&0x0f); 
			 }
